...
class SCOT(object):
	...
	def __init__(self, domain1: np.ndarray, domain2: np.ndarray):
		self.initialize_class(domain1, domain2)
	def initialize_class(self, domain1: np.ndarray, domain2: np.ndarray):
		"""
        Initializes the class instance with the given domains and sets default values for attributes.

        Parameters
        ----------
        domain1 : np.ndarray
            The first domain to be aligned.
        domain2 : np.ndarray
            The second domain to be aligned.
        """
		self.X=domain1
		self.y=domain2
	...
	def align(self, k: int, e: float, mode: str = "connectivity", metric: str = "correlation", verbose: bool = True, normalize: bool = False, norm: str = "l2", XontoY: bool = True, selfTune: bool=False, init_marginals: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        XontoY : bool, optional
            Direction of barycentric projection. Default is True.
        init_marginals : bool, optional
            Whether to initialize marginal distributions. Default is True.

        Returns
        -------
        Tuple[np.ndarray, np.ndarray]
            A tuple containing the aligned domain 1 and domain 2.
